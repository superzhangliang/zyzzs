...
!(function($){
	...
	$.fn.tabSelecter = function (options) {
		...
		this.close = function (id) {
			$(".imgcontent").removeAttr("style");//显示首页图片
			//隐藏并移除tab项
			var tab = this.getTab(id);
			if(!tab)return false;
			var mark = tab.mark,
		...
		this.newTab = function (id,text,src,isSwitch,needClose) {
			...
			if( text.indexOf("</i>") != -1 ){
				text = text.substring(text.indexOf("</i>")+4);
			}
		...
		this.switchTo = function (id) {
			...
			this.mark_list_item.each(function(num){
				...
				if(target==id){
					...
					//如果存在页面选择重载方法,则执行
		            if ('${currentPlat.serviceType}' == 1 && document.getElementById(target) && document.getElementById(target).contentWindow 
		            		&& document.getElementById(target).contentWindow.tabReload) {
		            	document.getElementById(target).contentWindow.tabReload();
		            }
