...
function deleteFile(aid,delFileName,haids){
	$.ajax({
		url:$("#basepath").val()+'message/deleteFile.do',
		dataType:'json',
		data:{id:aid},
...
function getAttachmentLi(item) {
	...
	if (item != undefined) {
		...
		+'<a href="javascript:void(0);"  onclick="downMove(this,\'li\');"  style="margin-right:10px;"title="下移"><i class="fa fa-arrow-down" ></a></i>'
		+'<a href="javascript:void(0);" onclick="deleteFile(\''+item.id
		+'\',\''+item.fileName+'\');" title="删除"><i class="fa fa-trash" ></i></a></li>';
	}
	return li;
}
...
function initMultiUpload(btnId, progressId, showSuccessId, successValId, type) {
	...
	var uploader = WebUploader.create({
	    ...
	    // 文件接收服务端。
	    server: $("#basepath").val() + 'message/multiUpload.do',
	...
	uploader.on( 'uploadSuccess', function( file,response ) {
		...
		var htm = getAttachmentLi(response);
		//alert(htm);
