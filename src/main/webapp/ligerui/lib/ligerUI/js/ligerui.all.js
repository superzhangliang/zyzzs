...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
            var g = this, p = this.options;
            var treeWidth = g.maxOutlineLevel * 25;
            if (p.checkbox) treeWidth += 25;
            if (p.parentIcon || p.childIcon) treeWidth += 25;
            treeWidth += p.nodeWidth; 
            g.tree.width("100%");//已修改,原为: g.tree.width(treeWidth);
        },
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (g.hasChildren(o))
                {
                    ...
                    {
                    	if (o.ischecked=="none")
                        	treehtmlarr.push('<div class=""></div>');
                        else{
                        	if(o.ischecked==true)
