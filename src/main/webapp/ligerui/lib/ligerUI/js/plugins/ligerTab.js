    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            $("> div", g.tab.content).each(function (i, box)
            {
                ...
                var itemTitle = contentitem.attr("title");
        ...
        {
            var g = this, p = this.options;
            g.droptip = g.droptip || $("<div class='l-tab-drag-droptip' style='display:block'><div class='l-drop-move-up'></div><div class='l-drop-move-down'></div></div>").appendTo('body');
            var drag = $(tabItemDom).ligerDrag(
            {
                ...
                proxy: function ()
                {
                    ...
                    g.dragproxy = $("<div class='l-tab-drag-proxy' style='display:block'><div class='l-drop-icon l-drop-no'></div></div>").appendTo('body');
        ...
        selectTabItem: function (tabid)
        {
            ...
            if (g.trigger('beforeSelectTabItem', [tabid]) == false)
                return false;
            g.selectedTabId = tabid;
            $("> .l-tab-content-item[tabid=" + tabid + "]", g.tab.content).show().css({visibility:"visible",position:"absolute"}).siblings().css({visibility:"hidden",display:"block",position:"absolute"})/*.hide(1,function(){$(this).css({visibility:"hidden"})})*/;
            $("li[tabid=" + tabid + "]", g.tab.links.ul).addClass("l-selected").siblings().removeClass("l-selected");
            g.trigger('afterSelectTabItem', [tabid]);
            //如果存在页面选择重载方法,则执行
            if (document.getElementById(tabid) && document.getElementById(tabid).contentWindow 
            		&& document.getElementById(tabid).contentWindow.tabReload) {
            	document.getElementById(tabid).contentWindow.tabReload();
            }
        },
        //移动到最后一个tab
        ...
        addTabItem: function (options)
        {
            ...
            var contentitem = $("<div class='l-tab-content-item'><div class='l-tab-loading' style='display:block;'></div><iframe frameborder='0'><html><head></head><body></body></html></iframe></div>");
            var iframeloading = $("div:first", contentitem);
            var iframe = $("iframe:first", contentitem);
            if (g.makeFullHeight){
                //var newheight = g.tab.height() - g.tab.links.height();
                //contentitem.height(newheight);
            	var setH = g.element.offsetHeight - g.tab.links[0].offsetHeight;
                contentitem.height(setH);
                g.tab.content.height(setH)
            }
            tabitem.attr("tabid", tabid).attr("title",options.text);
            contentitem.attr("tabid", tabid); 
            if (url)
            {
        ...
        removeTabItem: function (tabid)
        {
            ...
            {
                $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).prev().show().css({visibility:"visible"});
