...
(function ($)
{
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
			...
			if (g.dialog.body.find(".l-dialog-loading:first").length == 0)
                        ...
                        */
                        g.jiframe.attr("src", p.url).bind('load.dialog', function ()
                        {
                            iframeloading.hide();
                            //弹窗iframe添加隐藏input
                            var inputStr = '<input type="text" style="display:none" id="dialogID" value="'+dialogID+'"/>';
                            //$(this.contentDocument.body).prepend(inputStr);
                            var doc = this.contentDocument?this.contentDocument:this.contentWindow.document,
                            	input = document.createElement("input");
                            input.type= "text";
                            input.style.display="none";
                            input.value = dialogID;
                            input.id="dialogID";
            ...
            {
            	//没有底部按钮栏
            	g.dialog.content.css({ height: p.height - g._borderY });
        ...
        doMax: function (slide)
        {
            ...
            height -= (p.buttons?g._borderY-6:2);
            if (slide)
            {
        ...
        close: function ()
        {
            ...
            //解决内容被删除的问题:将内容移动至body下
            $("body").prepend(p.target);
    ...
    $.ligerDialog.prompt = function (title, value, multi, callback, setting)
    {
        ...
        if(setting&&typeof (setting) == "object"){
        	p = $.extend(true,p,setting);
        }
        var pL = 26;
        target.css({width:p.width-pL});
        return $.ligerDialog(p);
    };
