...
(function ($) {
	...
	$.ligerDefaults.GridString = {
		errorMessage: '发生错误',
		//pageStatMessage: '显示从{from}到{to}，总 {total} 条 。每页显示：{pagesize}条信息',
		pageStatMessage: '共有 {total} 条记录,当前第 {page} 页,共 {pageCount} 页',
	...
	$.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function () {
			...
			gridhtmlarr.push("                		<div class='l-bar-group'>每页显示</div>");
			gridhtmlarr.push("                		<div class='l-bar-group l-bar-selectpagesize' title='每页显示条数'></div>");
			gridhtmlarr.push("                		<div class='l-bar-group'>条</div>");
			gridhtmlarr.push("                		<div class='l-bar-separator'></div>");
			gridhtmlarr.push("                		<div class='l-bar-group'>");
			gridhtmlarr.push("                    		<div class='l-bar-button l-bar-btnfirst' title='最前一页'><span></span></div>");
			gridhtmlarr.push("                    		<div class='l-bar-button l-bar-btnprev' title='上一页'><span></span></div>");
			gridhtmlarr.push("                		</div>");
			
			gridhtmlarr.push("                		<div class='l-bar-separator'></div>");
			gridhtmlarr.push("                		<div class='l-bar-group'>");
			gridhtmlarr.push("                     		<div class='l-bar-button l-bar-btnnext' title='下一页'><span></span></div>");
			gridhtmlarr.push("                    		<div class='l-bar-button l-bar-btnlast' title='最后一页'><span></span></div>");
			gridhtmlarr.push("                		</div>");
			gridhtmlarr.push("                		<div class='l-bar-separator'></div>");
			gridhtmlarr.push("                		<div class='l-bar-group'><span class='pcontrol'>转到<input type='text' size='4' value='1' style='width:20px;vertical-align: top;' maxlength='3' />页<span style='display:none'></span></span></div>");
			gridhtmlarr.push("                		<div class='l-bar-group'>");
			gridhtmlarr.push("                     		<div class='l-bar-button l-bar-btnload' title='跳转'>跳转</div>");
			gridhtmlarr.push("                		</div>");
			gridhtmlarr.push("                		<div class='l-bar-separator'></div>");
		...
		_setHeight: function (h) {
            ...
            if (h > 0)
            {
                ...
                g.gridview.height(h+g.gridheader.outerHeight());
            }
		...
		_calculateGridBodyHeight: function (h) {
            ...
            if (typeof h == "string" && h.indexOf('%') > 0)
            {
                if (p.inWindow)
                    ...
                    h = g.grid.parent().outerHeight() * parseInt(h) * 0.01;
                h -= g.gridview.offset().top;
            }
            if (p.title) h -= 24;
            if (p.usePager && (p.pagerRender || !p.scrollToPage)) h -= g.toolbar.height();
            if (p.totalRender) h -= 25;
            if (p.toolbar) h -= g.topbar.height();
		...
		loadData: function (loadDataParm) {
			...
			if (p.dataType == "local") {
				...
				g._convertTreeData();///////////////////////////////////////////////////////////////////////////修改 debugger ???
				g._showData();
			}
		...
		_getColumnWidth: function (column) {
			...
			if (!colwidth || colwidth == "auto") {
				...
				colwidth = parseInt((g.grid.width() - 25 - noAutoColumnWidth) / autoColumnNumber) - 1;
			}
		...
		_createHeaderCell: function (column) {
			...
			if (column['__rowSpan']) {
				...
				jcell.css("lineHeight",p.headerRowHeight * column['__rowSpan']+"px");
			} else {
				jcell.height(p.headerRowHeight)
				jcell.css("lineHeight",p.headerRowHeight+"px")
			}
			...
			//if(p.detail&&p.detail.looking&&column.isdetail){return '';}
			return jcell;
		},
		...
		_showData: function () {
			...
			//g.gridview1.find(".l-grid-row-cell-checkbox")
			//g.gridview2.find(".l-grid-row")[0].offsetHeight
			var div = g.gridview1.find("td .l-grid-row-cell-inner:first"),
				cListLen = div.length,
				rows = g.gridview2.find(".l-grid-row");
			function getRowHeight(i){
				return parseInt(rows[i].offsetHeight);
			}
			for(i=0;i<cListLen;i++){
				var newCBHeihgt = getRowHeight(i);
				$(div[i]).css({height:newCBHeihgt+"px","min-height":newCBHeihgt+"px","line-height":newCBHeihgt+"px"});
				$(div[i]).find(".l-grid-row-cell-btn-checkbox").css({"margin-top":"7px"})
			}
			g.trigger('SysGridHeightChanged');
			if (p.totalRender) {
		...
		_fixHeight: function () {
			...
			for (var rowid in g.records) {
				var cell1 = g.getCellObj(rowid, column1), cell2 = g.getCellObj(rowid, column2),
					h_c1 = Math.ceil(cell1.offsetHeight), h_c2 = Math.ceil(cell2.offsetHeight),
					hIn_c1 = Math.ceil($(cell1).find(".l-grid-row-cell-inner").height()),
					hIn_c2 = Math.ceil($(cell2).find(".l-grid-row-cell-inner").height());
				var height = Math.max(h_c1, hIn_c1, h_c2, hIn_c2);
		...
		_getHtmlFromData: function (data, frozen) {
			...
			for (var i = 0, l = data.length; i < l; i++) {
				...
				$(g.columns).each(function (columnindex, column) {
					...
					//如果是行序号(系统列)
					if (this.isrownumber) {
						gridhtmlarr.push('<td id="' + g._getCellDomId(item, this) + '"');
						gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-rownumbers" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner l-xxxxxxxx-1"');
						if (p.fixedCellHeight)
							gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
						else
							gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px; height:'+ p.rowHeight +'px " ');
						gridhtmlarr.push('>' + (parseInt(item['__index']) + 1) + '</div></td>');//注入行序号
						return;
					}
					//如果是复选框(系统列)
					if (this.ischeckbox) {
						gridhtmlarr.push('<td id="' + g._getCellDomId(item, this) + '"');
						gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-checkbox" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner  l-xxxxxxxx-2"');
						if (p.fixedCellHeight)
							gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
						else
							gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px; height:'+ p.rowHeight +'px " ');
						gridhtmlarr.push('>');
						gridhtmlarr.push('<span class="l-grid-row-cell-btn-checkbox"></span>');//注入复选框
					...
					//如果是明细列(系统列)
						...
						gridhtmlarr.push('<td id="' + g._getCellDomId(item, this) + '"');
						gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-detail" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner l-xxxxxxxx-3"');
						if (p.fixedCellHeight)
							gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
						else
							gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px; height:'+ p.rowHeight +'px " ');
						...
						if (!p.isShowDetailToggle || p.isShowDetailToggle(item)) {
							gridhtmlarr.push('<span class="l-grid-row-cell-detailbtn"></span>');//注入明细按钮
						}
						gridhtmlarr.push('</div></td>');
						return;
					}else{
						var colwidth = this._width;
						if(column.columnindex==0&&!column.columnname){return;}
						gridhtmlarr.push('<td id="' + g._getCellDomId(item, this) + '"');
						gridhtmlarr.push(' class="l-grid-row-cell ');
						if(p.detail&&p.detail.looking == column.columnname){
							gridhtmlarr.push(' l-grid-looking ');
						}
						...
						gridhtmlarr.push(' ">');
						gridhtmlarr.push(g._getCellHtml(item, column));//注入其他内容
						gridhtmlarr.push('</td>');
						return;
					}
				});
		...
		_getCellHtml: function (rowdata, column) {
			...
			//if (p.fixedCellHeight) htmlarr.push('height:' + p.rowHeight + 'px;');//取消固定高度
			//htmlarr.push('height:' + p.rowHeight + 'px;');//添加备用高度 new
			htmlarr.push(' position:relative; min-height:' + p.rowHeight + 'px;line-height:'+p.rowHeight+"px; ");
			...
			//htmlarr.push('" title="'+value+'">' + content + '</div>');
			return htmlarr.join('');
		},
		...
		_buildPager: function () {
			...
			stat = stat.replace(/{from}/, r1);//从第n页开始
			stat = stat.replace(/{to}/, r2);//从第m页结束
			stat = stat.replace(/{total}/, p.total);//总共q条
			stat = stat.replace(/{pagesize}/, p.pageSize);//每页p条信息
			stat = stat.replace(/{page}/, p.page);//当前第x页
			stat = stat.replace(/{pageCount}/, p.pageCount);//共y页
		...
		_onClick: function (e) {
			...
			if (src.checkboxall) {/*复选框全选*/
			...
			} else if (src.detailbtn && p.detail) {//明细
				var item = src.data;
				...
				if ($(src.detailbtn).hasClass("l-open")) {
					...
					$(src.row).find('.l-grid-looking').removeClass("l-open");
				} else {
					...
					if (nextrow.length > 0) {
						...
						$(src.row).find('.l-grid-looking').addClass("l-open");
						g.trigger('SysGridHeightChanged');
						return;
					}
					$(src.detailbtn).addClass("l-open");
				...
				g.trigger('SysGridHeightChanged');
			} else if (src.treelink) {//树 - 伸展/收缩节点
				g.toggle(src.data);
			...
			}else if($(src.cell).hasClass('l-grid-looking')){//监听列
				var item = src.data;
				var row = $([g.getRowObj(item, false)]);
				if (g.enabledFrozen()) row = row.add(g.getRowObj(item, true));
				var rowid = item['__id'];
				if ($(src.row).find('.l-grid-looking').hasClass("l-open")) {
					if (p.detail.onCollapse)
						p.detail.onCollapse(item, $(".l-grid-detailpanel-inner:first", nextrow)[0]);
					row.next("tr.l-grid-detailpanel").hide();
					$(src.cell).removeClass("l-open");
					$(src.row).find('.l-grid-row-cell-detailbtn').removeClass("l-open");
				} else {
					var nextrow = row.next("tr.l-grid-detailpanel");
					if (nextrow.length > 0) {
						nextrow.show();
						if (p.detail.onExtend)
							p.detail.onExtend(item, $(".l-grid-detailpanel-inner:first", nextrow)[0]);
						$(src.cell).addClass("l-open");
						$(src.row).find('.l-grid-row-cell-detailbtn').addClass("l-open");
						g.trigger('SysGridHeightChanged');
						return;
					}
					$(src.cell).addClass("l-open");
			...
			} else if (src.row && !g.enabledCheckbox()) {//其他
		...
		_onResize: function () {
			...
			if (g.enabledFrozen()) {
				...
				//根据实际内容高度,调节checkbox列对应项的高度
				$(g.gridview1).find(".l-grid-row-cell-inner").each(function(i){
					var rowH = $(g.gridview2).find(".l-grid-row").eq(i).height()-1;
					$(this).height(rowH);
				})
			}
